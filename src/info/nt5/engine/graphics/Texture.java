...
public class Texture {
	...
	public static Texture fromColor(Color c, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				buffer.put((byte) (c.r * 255.0f)).put((byte) (c.g * 255.0f)).put((byte) (c.b * 255.0f))
						.put((byte) (c.a * 255.0f));
			}
		}
	...
	public static Texture fromByteBuffer(ByteBuffer buffer, int width, int height, int components) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, components == 4 ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE,
				buffer);
		texture.unbind();
